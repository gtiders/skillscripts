...
pub(crate) struct ConfigSnapshot {
    pub(crate) built_in_defaults: Config,
...
impl ConfigResolver {
    ...
    fn load_optional_config(path: &Path) -> Result<Option<Config>> {
        if !path.exists() {
            return Ok(None);
        }
...
fn resolve_scan_path(base_dir: &Path, home: &Path, scan_path: &str) -> String {
    ...
    candidate.clean().to_string_lossy().into_owned()
}
...
pub(crate) fn resolve_config_snapshot(local_dir: &Path) -> Result<ConfigSnapshot> {
    let built_in_defaults = Config::default();
    let global_config_path = get_global_config_dir().join(CONFIG_FILE_NAME);
    let global_config = if global_config_path.exists() {
        Some(load_config_file(&global_config_path)?)
    } else {
        None
    };
